...
function CohortOptionsParams({ stateParams, setValue }: VisOptionsProps<CohortVisParams>) {
  ...
  const onChangePivottable = useCallback(
    (value: CohortVisParams['pivottable']) => setValue('pivottable', value),
    [setValue]
  );

  const onChangeSampleTable = useCallback(
    (value: CohortVisParams['sample_table']) => setValue('sample_table', value),
    [setValue]
  );
  ...
  return (
    <Fragment>
      ...
      <EuiCheckbox
        ...
        checked={stateParams.pivottable}
        onChange={({ target: { checked } }) => onChangePivottable(Boolean(checked))}
      />
      <EuiCheckbox
        id="sample_table"
        label="Use Sample EUI Table"
